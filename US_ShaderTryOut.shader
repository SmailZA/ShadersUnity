Shader "Unlit/US_ShaderTryOut"
{
   ...
   //             v2f o;
			...
			//	//ticking stripes
			//	v.uv.x = floor(sin(v.vertex.x * _Time.y) * cos(v.vertex.y * _Time.x));
			//	
   ...
   //         }
			...
			//	float m = smoothstep(.3, .0, d);
			//	distort = distort * 20 * m;

			//	float glow = saturate(1 - distort);

			//	float4 col = tex2D(_MainTex, i.uv + distort);
